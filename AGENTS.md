...
Session recordings, replays, live view, and logs come from the core Browserbase SDK (@browserbasehq/sdk) — not Stagehand. Before building any observability feature, consult Browserbase's observability docs: https://docs.browserbase.com/platform/browser/observability

Session replay specifically — retrieving a session's recording as an HLS playlist — is documented here: https://docs.browserbase.com/platform/browser/observability/session-replay The retrieval needs the secret API key, so it must be proxied server-side.
...
# React Flow documentation is the source of truth

Whenever working with the React Flow API, components, or usage in general, do not rely on training data. Search and follow the current guidance in https://reactflow.dev/llms.txt instead.
